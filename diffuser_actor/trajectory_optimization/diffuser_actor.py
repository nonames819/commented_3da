...
class DiffuserActor(nn.Module):
    ...
    def convert_rot(self, signal):
        signal[..., 3:7] = normalise_quat(signal[..., 3:7]) # signal: torch.Size([36, 1, 7])
        if self._rotation_parametrization == '6D':
        ...
        return signal # signal: torch.Size([36, 1, 9]) chd: 为了方便神经网络学习的连续表示
    ...
    def convert2rel(self, pcd, curr_gripper):
        ...
        center = curr_gripper[:, -1, :3]  # (batch_size, 3) chd: 就是当前夹爪的三维坐标
        bs = center.shape[0]
        pcd = pcd - center.view(bs, 1, 3, 1, 1) # chd: 计算点云相对kp中心的相对坐标
        curr_gripper = curr_gripper.clone()
        curr_gripper[..., :3] = curr_gripper[..., :3] - center.view(bs, 1, 3) # chd: 计算（可能有hist）的proprio输入的相对坐标
    ...
    def forward(
        ...
        """
        Arguments:
            gt_trajectory: (B, trajectory_length, 3+4+X) chd: 相当于action
        ...
        pcd_obs = torch.permute(self.normalize_pos( 
            torch.permute(pcd_obs, [0, 1, 3, 4, 2])
        ), [0, 1, 4, 2, 3]) # torch.Size([36, 4, 3, 256, 256])
        curr_gripper[..., :3] = self.normalize_pos(curr_gripper[..., :3])

        # Convert rotation parametrization
        gt_trajectory = self.convert_rot(gt_trajectory) # chd: torch.Size([36, 1, 9])
        curr_gripper = self.convert_rot(curr_gripper)
        ...
        # Condition on start-end pose TODO
        ...
        noisy_trajectory[cond_mask] = cond_data[cond_mask]  # condition TODO
