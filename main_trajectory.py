...
class TrainTester(BaseTrainTester):
    ...
    def train_one_step(self, model, criterion, optimizer, step_id, sample):
        ...
        if step_id % self.args.accumulate_grad_batches == 0:
            optimizer.zero_grad()
        # chd: 直接对sample进行操作
        if self.args.keypose_only: # chd: 这里预测keypose的话使用的是只有下一个kp的traj，长度为1
            sample["trajectory"] = sample["trajectory"][:, [-1]] # torch.Size([36, 1, 8])~num_ep, 1, 8
            sample["trajectory_mask"] = sample["trajectory_mask"][:, [-1]] # torch.Size([36, 1])
        else: # chd: 这里不预测keypose的话使用的是不包含开头的traj
