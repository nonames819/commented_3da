...
class RLBenchDataset(Dataset):
    ...
    def __init__(
        ...
        if return_low_lvl_trajectory:
            ...
            self._interpolate_traj = TrajectoryInterpolator(
                ...
                interpolation_length=interpolation_length # 固定插值长度
            )
        ...
        for task, eps in episodes_by_task.items():
            ...
            self._episodes += eps # eps是每一个任务的所有ep
            self._num_episodes += len(eps) 
            print(f'task: {task} num_episodes: {len(eps)}')
    ...
    def __getitem__(self, episode_id):
        ...
        # Dynamic chunking so as not to overload GPU memory 下面会选取self._max_episode_length数量的分块，因此(chunk + 1) * self._max_episode_length不能超过episode切割出来的分块数量
        ...
        states = torch.stack([
            ...
            for i in frame_ids
        ]) # torch.Size([5, 4, 2, 3, 256, 256]) self._max_episode_length, num_cam, 2, 3, h, w 2是指RGB图和深度图
        ...
        if episode[3]:
            ...
            states = states[:, index] # 按照self._cameras顺序排个序
        ...
        action = torch.cat([episode[2][i] for i in frame_ids]) #(self._max_episode_length, 8) 8 = 3+4+1

        # Sample one instruction feature
        if self._instructions:
            instr = random.choice(self._instructions[task][variation]) # torch.Size([53, 512])
            instr = instr[None].repeat(len(rgbs), 1, 1) # torch.Size([5, 53, 512])
        else:
            instr = torch.zeros((rgbs.shape[0], 53, 512))

        # Get gripper tensors for respective frame ids
        gripper = torch.cat([episode[4][i] for i in frame_ids]) # torch.Size([5, 8])
        ...
        if self._return_low_lvl_trajectory:
            if len(episode) > 5:
                traj_items = [
                    self._interpolate_traj(episode[5][i]) for i in frame_ids # 传入原始轨迹，返回插值后的轨迹，返回frame_ids个茶之后的轨迹
                ]
            else:
                traj_items = [
        ...
        ret_dict = {
            ...
            "rgbs": rgbs,  # e.g. tensor (n_frames, n_cam, 3+1, H, W) TODO: why +1?
